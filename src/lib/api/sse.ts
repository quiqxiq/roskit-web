...
//
// Auth token is appended as `?token=` query param because EventSource
// cannot send custom headers. Backend explicitly accepts this per
// `docs/openapi/paths/sse.yaml`.
export function useSSE<T = unknown>(
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const url = `${baseURL}/api/v1${path}${sep}token=${encodeURIComponent(token)}`
